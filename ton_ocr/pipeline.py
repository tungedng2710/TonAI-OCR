...
class TonOCRPipeline:
    def __init__(self,
                 box_thresh=0.6, 
                 unclip_ratio=1.6,
                 text_det_onnx_model=f"{os.path.dirname(__file__)}/models/text_detection.onnx",
                 text_rec_onnx_model=f"{os.path.dirname(__file__)}/models/text_recognition.onnx",
                 text_rec_dict="vocabs.txt"):
    ...
    def predict(self, img, drop_score=0.5, unclip_ratio=None, box_thresh=None):
        ori_im = img.copy()
        dt_boxes, _ = self.text_detector(img, unclip_ratio, box_thresh)
        if dt_boxes is None:
            return []
        img_crop_list = []
